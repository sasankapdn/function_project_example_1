...
//                    .build();
            JSONObject json = readJsonFromUrl(objectInfo.content.toString());
            JSONArray docs = json.getJSONArray("objects");
            ...
            PutObjectRequest por = PutObjectRequest.builder()
                    ...
                    .bucketName(objectInfo.bucketName)
                    .objectName(objectInfo.name)
                    .putObjectBody(new ByteArrayInputStream(csv.getBytes(StandardCharsets.UTF_8)))
                    .build();
